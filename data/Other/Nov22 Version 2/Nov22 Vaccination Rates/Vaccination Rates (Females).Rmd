---
title: "Vaccination Rates (Females)"
author: "Tori Diamond"
date: '2022-06-23'
output: pdf_document
---
...
ggplot(subset(HSUvsERP_NMFVacc_DHB_SFtot.df), aes(x=factor(AgeGroup, level= level_order), y = RateMult, fill = population, group=population)) +
  geom_col(position = position_dodge()) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5,hjust=1)) +
  geom_errorbar(aes(ymin=Rate_Gamma1Lwr, ymax=Rate_Gamma1Upr), position = position_dodge(width = 0.85), width = 0.5) +
  facet_wrap(~DHB) +
  plot_annotation(title = "Non-Maori Females Fully Vaccinated by Age Groups") +
  labs(y= "Rate per 100,000",
       x= "Age Groups") &
  theme(plot.title = element_text(hjust = 0.5))
...
```{r, eval=FALSE}
#pdf(file="Non-Maori Females Fully Vaccinated by Age Groups.pdf", width = 10, height = 10)
...
## Rate ratio for Non-Maori Females Fully Vaccinated Rates by grouped DHB with 95% CI's
```{r, fig.height= 10, fig.width= 10}


ggplot(subset(NMFVacc_DHB_SFpopulation.df, DHB == "Total"), aes(x=factor(AgeGroup, level= level_order), y = RelativeRisk, fill=RelativeRisk))+
  geom_col(fill = "Steel Blue 2")+
  geom_errorbar(aes(min=RelativeRiskLwr, ymax=RelativeRiskUpr), width=0.5) +
  plot_annotation(title = "Non-Maori Females Fully Vaccinated Rate Ratio by Age Group (HSU as baseline)") +
  labs(y= "Rate Ratio",
       x= "Age Groups") &
  theme(plot.title = element_text(hjust = 0.5))
...
```{r, eval=FALSE}
#pdf(file="Non-Maori Females Fully Vaccinated Rate Ratio by Age Group (HSU as baseline).pdf", width = 10, height = 10)
...
# Total Females fully vaccinated rates by Age (across all DHB)
...
# Non-Maori Females fully vaccinated rates by Age (across all DHB)


```{r, fig.height= 10, fig.width= 10}

HSUvsERP_NMFVacc_SFnontot.df <- rbind(
  data.frame(HSU_NMFVacc_SFnontot.df, population="HSU"),
  data.frame(ERP_NMFVacc_SFnontot.df, population="ERP")
)

ggplot(subset(HSUvsERP_NMFVacc_SFnontot.df), aes(x=factor(AgeGroup, level= level_order), y = RateMult, fill = population, group=population)) +
  geom_col(position = position_dodge()) +
  theme(axis.text.x=element_text(angle=90,vjust=0.5,hjust=1)) +
  geom_errorbar(aes(ymin=Rate_Gamma1Lwr, ymax=Rate_Gamma1Upr), position = position_dodge(width = 0.85), width = 0.5) +
  facet_wrap(~DHB) +
  plot_annotation(title = "Non-Maori Females Fully Vaccinated by Age Group & DHB") +
  labs(y= "Rate per 100,000",
       x= "Age Groups") &
  theme(plot.title = element_text(hjust = 0.5))
...
```{r, eval=FALSE}
#pdf(file="Non-Maori Females Fully Vaccinated by Age Group & DHB Fullplot.pdf", width = 10, height = 10)
...
# Maori Females fully vaccinated rates by Age, in depth for each DHB
